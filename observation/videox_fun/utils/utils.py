        ...
        if isinstance(input_video_path, str):
            ...
            while True:
                ...
                if frame_count % frame_skip == 0:
                    ...
                    # Only the first `video_length` (post-skip) frames are ever used
                    # (sliced below). Stop early so a long control video isn't fully
                    # decoded into RAM — variable-length mh clips (up to ~16min @30fps,
                    # tens of GB) otherwise OOM the process here.
                    if video_length is not None and len(input_video) >= video_length:
                        break
